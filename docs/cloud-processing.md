...
Remote mode transfers a selected recording to private object storage, then transfers a labeled transcript snapshot for summary generation. Generated speaker embeddings are returned to the Mac and names enter the remote text snapshot only after local review. The server never receives a Mac path or the local roster. Closing the app does not cancel an accepted job; choosing **Process locally** explicitly fences/cancels the remote run before enqueueing local work.
